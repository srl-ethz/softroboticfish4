...
class BaseRtlSdr(object):
    ...
    def read_register(self, block, address, length):
        result = librtlsdr.rtlsdr_read_reg(self.dev_p, block, address, length)
        return result

    def write_register(self, block, address, length, value):
        result = librtlsdr.rtlsdr_write_reg(self.dev_p, block, address, value, length)
        if result < 0:
            raise IOError('Error code %d when getting tuner type'\
                          % (result))
    ...
    def set_gpio_bit(self, bit, value):
        librtlsdr.rtlsdr_set_gpio_bit(self.dev_p, bit, value)

    def set_gpio_output(self, bit):
        librtlsdr.rtlsdr_set_gpio_output(self.dev_p, bit)
